...
dynamicarray_int *dynamicarray_int_alloc(size_t capacity){
    if (capacity == 0){
        Rprintf("%s: line %d: `dynamicarray_int_alloc` capacity must be at least one, now is zero.\n", __FILE__, __LINE__);
        return NULL;
    }
...
int dynamicarray_int_get(dynamicarray_int *array, size_t i){
    if (i >= array->capacity){
        error("%s: line %d: `dynamicarray_int_get` index %zu is out of range for array of capacity %zu.\n", __FILE__, __LINE__, i, array->capacity);
        // exit(EXIT_FAILURE);
    }
    return array->data[i];
}
...
void test_dynamicarray_int_contains(void){
    ...
    Rprintf("Tests for `dynamicarray_int_contains` completed: no issues found.\n");
}
...
void test_dynamicarray_equal(void){
    ...
    dynamicarray_int_append(a, 12);
    assert(dynamicarray_int_equal(a, b)==0);
    Rprintf("Tests for `dynamicarray_int_equal` completed: no issues found.\n");
}
...
void test_dynamicarray_int_unique_elements(void){
    ...
    Rprintf("Tests for `dynamicarray_int_unique_elements` completed: no issues found.\n");
}
...
void test_dynamicarray_int_permute_equal(void){
    ...
    assert(dynamicarray_int_permute_equal(a, b)==1);
    Rprintf("Tests for `dynamicarray_int_permute_equal` completed: no issues found.\n");
}
...
void test_dynamicarray(void){
    ...
    Rprintf("=== Tests: all tests for `dynamicarray` completed: no issues found.\n");
}