...
void test_cm_int_min(void){
    ...
    assert(cm_int_min(-3, -4) == -4);
    Rprintf("Tests for `cm_int_min` completed: no issues found.\n");
}
...
void test_cm_int_max(void){
    ...
    assert(cm_int_max(-3, -4) == -3);
    Rprintf("Tests for `cm_int_max` completed: no issues found.\n");
}
...
void test_cm_d_min(void){
    ...
    Rprintf("Tests for `cm_d_min` completed: no issues found.\n");
}
...
void test_cm_d_max(void){
    ...
    Rprintf("Tests for `cm_d_max` completed: no issues found.\n");
}
...
void test_cm_abs_distance(void){
    ...
    y = -0.8;
    assert((cm_abs_distance(x, y) == cm_abs_distance(y, x)) && (cm_abs_distance(x, y) >= 0));
    Rprintf("Tests for `cm_abs_distance` completed: no issues found.\n");
}
...
void test_vector_sort_index(void){
    ...
    condv4 = (vector_get(v, 3) == v3);
    assert(cond1 && cond2 && cond3 && cond4 && condv1 && condv2 && condv3 && condv4);
    Rprintf("Tests for `vector_sort_index` completed: no issues found.\n");
}
...
int cm_cmmodel_init_check_values(CMModel *cmm, int test_mode){
    ...
    if (!((cmm->d->size == cmm->y->size) && (cmm->y->size == cmm->x->size1))){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: dimension mismatch between `y`, `d`, `x`.\n", __FILE__, __LINE__);
        }
    ...
    if (cmm->theta->size != (cmm->x->size2 + 1)){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: length of `theta` must be larger than that of `x` exactly by one; current length of `theta` is %zu.\n", __FILE__, __LINE__, cmm->theta->size);
        }
    ...
    if (cmm->delta < 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: caliper `delta` must be zero to use default value, or be srictly positive; now it is %f.\n", __FILE__, __LINE__, cmm->delta);
        }
    ...
    for (int i = 0; i < cmm->d->size; i++){
        if (((int)vector_short_get(cmm->d, i) != 0) && ((int)vector_short_get(cmm->d, i) != 1)){
            if (!test_mode)
            {
                Rprintf("%s: line %d: CMModel input check failed: treatment vector `d` must contain elements zero or one; d[%d] has value %d.\n", __FILE__, __LINE__, i, (int)vector_short_get(cmm->d, i));
            }
    ...
    if (nr_treated == 0 || nr_treated == cmm->d->size){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: imbalanced number of treated and control units; there are %d treated units, %lu control units.\n", __FILE__, __LINE__, nr_treated, (cmm->d->size) - nr_treated);
        }
    ...
    if ((cmm->beta < 0) || (cmm->beta >= 1.0 / 4)){
        if (!test_mode)
        {
            Rprintf("%s: line %d: CMModel input check failed: 0 <= `beta` < 1/4 must be; now it is %f.\n", __FILE__, __LINE__, cmm->beta);
        }
    ...
    if ((cmm->alpha < 0) || (cmm->alpha >= 1.0 / 4)){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: 0 <= `alpha` < 1/4 must be; now it is %f.\n", __FILE__, __LINE__, cmm->alpha);
        }
    ...
    if ((cmm->alpha > 0) && (cmm->beta > 0) && (cmm->alpha >= cmm->beta)){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: 0 < `alpha` < `beta` must be or at least one of `alpha` or `beta` must be zero; now alpha is %f, beta is %f.\n", __FILE__, __LINE__, cmm->alpha, cmm->beta);
        }
    ...
    if (cmm->kappa_a < 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: 0 <= `kappa_a` must be; now kappa_a is %f.\n", __FILE__, __LINE__, cmm->kappa_a);
        }
    ...
    if (cmm->kappa_gamma < 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: 0 <= `kappa_gamma` must be; now kappa_gamma is %f.\n", __FILE__, __LINE__, cmm->kappa_gamma);
        }
    ...
    if (cmm->kappa_gamma_derivative < 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: 0 <= `kappa_gamma_derivative` must be; now kappa_gamma_derivative is %f.\n", __FILE__, __LINE__, cmm->kappa_gamma_derivative);
        }
...
int cm_cmmodel_check_values(CMModel *cmm, int test_mode){
    ...
    if (cmm->delta <= 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModel input check failed: caliper `delta` must be strictly positive; now it is %f.\n", __FILE__, __LINE__, cmm->delta);
        }
        return CMModel_ERROR_CALIPER;
    }
    // treatment vector: zero/one check
...
void test_cm_cmmodel_check_values(void){
    ...
    matrix_free(x);
    vector_free(theta);
    Rprintf("Tests for `cm_cmmodel_check_values` completed: no issues found.\n");
}
...
int cm_cmmodelknownpropscore_init_check_values(CMModelKnownPropscore *cmm, int test_mode){
    ...
    if (!((cmm->d->size == cmm->y->size) && (cmm->y->size == cmm->propscore->size))){
        if (!test_mode){
            Rprintf("%s: line %d: CMModelKnownPropscore input check failed: input check failed: dimension mismatch between `y`, `d`, `propscore` or length unequal to `n`.\n", __FILE__, __LINE__);
        }
    ...
    if (cmm->delta < 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModelKnownPropscore input check failed: caliper `delta` must be zero to use default value, or be srictly positive; now it is %f.\n", __FILE__, __LINE__, cmm->delta);
        }
    ...
    for (int i = 0; i < cmm->d->size; i++){
        if (((int)vector_short_get(cmm->d, i) != 0) && ((int)vector_short_get(cmm->d, i) != 1)){
            if (!test_mode)
            {
                Rprintf("%s: line %d: CMModelKnownPropscore input check failed: treatment vector `d` must contain elements zero or one; d[%d] has value %d.\n", __FILE__, __LINE__, i, (int)vector_short_get(cmm->d, i));
            }
    ...
    if (nr_treated == 0 || nr_treated == cmm->d->size){
        if (!test_mode){
            Rprintf("%s: line %d: CMModelKnownPropscore input check failed: imbalanced number of treated and control units; there are %d treated units, %lu control units.\n", __FILE__, __LINE__, nr_treated, (cmm->d->size) - nr_treated);
        }
    ...
    for (int i = 0; i < cmm->propscore->size; i++){
        if (!((0 < vector_get(cmm->propscore, i)) && (vector_get(cmm->propscore, i) < 1)))
        {
            if (!test_mode)
            {
                Rprintf("%s: line %d: CMModelKnownPropscore input check failed: propensity score `propscore` must contain elements strictly between zero or one; propscore[%d] has value %f.\n", __FILE__, __LINE__, i, vector_get(cmm->propscore, i));
            }
    ...
    if ((cmm->beta < 0) || (cmm->beta >= 1.0 / 4)){
        if (!test_mode)
        {
            Rprintf("%s: line %d: CMModelKnownPropscore input check failed: 0 <= `beta` < 1/4 must be; now it is %f.\n", __FILE__, __LINE__, cmm->beta);
        }
    ...
    if ((cmm->alpha < 0) || (cmm->alpha >= 1.0 / 4)){
        if (!test_mode){
            Rprintf("%s: line %d: CMModelKnownPropscore input check failed: 0 <= `alpha` < 1/4 must be; now it is %f.\n", __FILE__, __LINE__, cmm->alpha);
        }
    ...
    if ((cmm->alpha > 0) && (cmm->beta > 0) && (cmm->alpha >= cmm->beta)){
        if (!test_mode){
            Rprintf("%s: line %d:  CMModelKnownPropscore input check failed: 0 < `alpha` < `beta` must be or at least one of `alpha` or `beta` must be zero; now alpha is %f, beta is %f.\n", __FILE__, __LINE__, cmm->alpha, cmm->beta);
        }
    ...
    if (cmm->kappa_a < 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModelKnwonPropscore input check failed: 0 <= `kappa_a` must be; now kappa_a is %f.\n", __FILE__, __LINE__, cmm->kappa_a);
        }
    ...
    if (cmm->kappa_gamma < 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModelKnwonPropscore input check failed: 0 <= `kappa_gamma` must be; now kappa_gamma is %f.\n", __FILE__, __LINE__, cmm->kappa_gamma);
        }
...
int cm_cmmodelknownpropscore_check_values(CMModelKnownPropscore *cmm, int test_mode){
    ...
    if (cmm->delta <= 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModelKnownPropscore input check failed: caliper `delta` must be strictly positive; now it is %f.\n", __FILE__, __LINE__, cmm->delta);
        }
        return CMModelKnownPropscore_ERROR_CALIPER;
    }
    // treatment vector: zero/one check
    ...
    if (cmm->kappa_gamma < 0){
        if (!test_mode){
            Rprintf("%s: line %d: CMModelKnownPropscore input check failed: 0 <= `kappa_gamma` must be; now kappa_gamma is %f.\n", __FILE__, __LINE__, cmm->kappa_gamma);
        }
        return CMModelKnownPropscore_ERROR_KAPPAGAMMA;
    }
    // all checks passed
...
void test_cm_cmmodelknownpropscore_check_values(void){
    ...
    vector_short_free(d);
    vector_free(propscore);
    Rprintf("Tests for `cm_cmmodelknownpropscore_check_values` completed: no issues found.\n");
}
...
void test_cm_compute_singleindex_score(void){
    ...
    matrix_free(x);
    vector_free(theta);
    Rprintf("Tests for `cm_compute_singleindex_score` completed: no issues found.\n");
}
...
void test_cm_compute_propscore(void){
    ...
    vector_free(propscore);
    vector_free(result);
    Rprintf("Tests for `cm_compute_propscore` completed: no issues found.\n");
}
...
void cm_matches(CMModelKnownPropscore *cm_model, CMResults *results){
    ...
    if (NUM_THREADS > 1){ // multithreaded version
        ...
        // Rprintf("Multithreading started, obs_per_thread = %d.\n", obs_per_thread);
        //  create threads
        int result_code;
        pthread_t threads[NUM_THREADS];
        for (long t = 0; t < NUM_THREADS; t++){
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_matches`: creating thread %ld.\n", t);
            }
            ...
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_matches`: thread %ld pscore_sortindex_high is %zu.\n", t, thread_args[t].pscore_sortindex_high);
            }
            assert(!result_code);
        }
        ...
        for (long t = 0; t < NUM_THREADS; t++){
            ...
            assert(!result_code);
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_matches`: thread %ld has finished.\n", t);
            }
        }
...
void test_cm_matches(void){
    ...
    Rprintf("Tests for `cm_matches` completed: no issues found.\n");
}
...
void test_cm_te_estimator_thread(void){
    ...
    // Rprintf("ate_hat = %f, results->ate_hat = %f\n", ate_hat, results->ate_hat);
    // Rprintf("att_hat = %f, results->att_hat = %f\n", att_hat, results->att_hat);
    ...
    free(cm_model);
    free(thread_args);
    Rprintf("Tests for `cm_te_estimator_thread` completed: no issues found.\n");
}
...
void cm_te_estimator(CMModelKnownPropscore *cm_model, CMResults *results){
    ...
    if (NUM_THREADS > 1){ // multithreading
        ...
        for (long t = 0; t < NUM_THREADS; t++){
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_te_estimator`: creating thread %ld.\n", t);
            }
            thread_args[t].cm_model = cm_model;
            ...
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_te_estimator`: thread %ld index_high is %zu.\n", t, thread_args[t].index_high);
            }
        ...
        for (long t = 0; t < NUM_THREADS; t++){
            ...
            assert(!result_code);
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_te_estimator`: thread %ld has finished.\n", t);
            }
        }
    } else { // single thread
        CMThreadArgumentsTE *thread_args = malloc(sizeof(CMThreadArgumentsTE));
...
void test_cm_compute_var_estpi(void){
    ...
    assert(fabs(result[0] - v_estpi[0]) <= pow(10, -CM_TEST_DOUBLE_TOLERANCE_NPOWER));
    assert(fabs(result[1] - v_estpi[1]) <= pow(10, -CM_TEST_DOUBLE_TOLERANCE_NPOWER));
    Rprintf("Tests for `cm_compute_var_estpi` completed: no issues found.\n");
}
...
void test_cm_update_fisher_info(void){
    ...
    Rprintf("Tests for `cm_update_fisher_info` completed: no issues found.\n");
}
...
int cm_update_qs(size_t i, vector *q_d0, vector *q_d0_treated, vector *q_d1, vector *q_d1_treated, CMModelKnownPropscore *cm_model, double p1_hat, double gamma_n, double gamma_derivative_n, double *h_d, double *qmu_d){
    ...
    // Rprintf("h_d_deriv_p[0] = %f; h_d_deriv_p[1] = %f.\n", h_d_deriv_p[0], h_d_deriv_p[1]);
    // Rprintf("qmu_d_deriv_p[0] = %f; qmu_d_deriv_p[1] = %f.\n", qmu_d_deriv_p[0], qmu_d_deriv_p[1]);
    // Rprintf("mu_d_deriv_p[0] = %f; mu_d_deriv_p[1] = %f.\n", mu_d_deriv_p[0], mu_d_deriv_p[1]);
    // Rprintf("deriv_p[0] = %f; deriv_p[1] = %f.\n", deriv_p[0], deriv_p[1]);
...
void test_cm_update_qs(void){
    ...
    for (int i=0; i<n; i++){ // test i=0,...,3
        ...
        mu_d_deriv_p[0] = (qmu_d_deriv_p[0] * h_d[0] - qmu_d[0] * h_d_deriv_p[0]) / (n * pow(h_d[0], 2));
        mu_d_deriv_p[1] = (qmu_d_deriv_p[1] * h_d[1] - qmu_d[1] * h_d_deriv_p[1]) / (n * pow(h_d[1], 2));
        // Rprintf("TEST: h_d_deriv_p[0] = %f, h_d_deriv_p[1] = %f \n", h_d_deriv_p[0], h_d_deriv_p[1]);
        // Rprintf("TEST: qmu_d_deriv_p[0] = %f; qmu_d_deriv_p[1] = %f.\n", qmu_d_deriv_p[0], qmu_d_deriv_p[1]);
        // Rprintf("TEST: mu_d_deriv_p[0] = %f; mu_d_deriv_p[1] = %f.\n", mu_d_deriv_p[0], mu_d_deriv_p[1]);
        //  entry 0
        j = 0;
        ...
        mu_d_deriv_thetaj[0] = (qmu_d_deriv_thetaj[0] * h_d[0] - qmu_d[0] * h_d_deriv_thetaj[0]) / (n * pow(h_d[0], 2));
        mu_d_deriv_thetaj[1] = (qmu_d_deriv_thetaj[1] * h_d[1] - qmu_d[1] * h_d_deriv_thetaj[1]) / (n * pow(h_d[1], 2));
        // Rprintf("TEST: h_d_deriv_thetaj[0] = %f; h_d_deriv_thetaj[1] = %f.\n", h_d_deriv_thetaj[0], h_d_deriv_thetaj[1]);
        // Rprintf("TEST: qmu_d_deriv_thetaj[0] = %f; qmu_d_deriv_thetaj[1] = %f.\n", qmu_d_deriv_thetaj[0], qmu_d_deriv_thetaj[1]);
        // Rprintf("TEST: mu_d_deriv_thetaj[0] = %f, mu_d_deriv_thetaj[1] = %f \n", mu_d_deriv_thetaj[0], mu_d_deriv_thetaj[1]);
    ...
    free(cm_model); 
    Rprintf("Tests for `cm_update_qs` completed: no issues found.\n");
}
...
void *cm_variance_estimator_thread(void *thread_arguments){
    ...
    if (CM_VERBOSE_MODE){
        Rprintf("pscore_min = %f, pscore_max = %f.\n", pscore_min, pscore_max);
    }
    ...
    for (size_t i = thread_args->index_low; i < thread_args->index_high; i++){
        ...
        if (cm_inrange(vector_get(thread_args->cm_model->propscore, i), truncation_low, truncation_high)){
            ...
            //
            // Rprintf("h_d[0] = %f; h_d[1] = %f.\n", h_d[0], h_d[1]);
            // Rprintf("qmu_d[0] = %f; qmu_d[1] = %f.\n", qmu_d[0], qmu_d[1]);
            // Rprintf("qmu2_d[0] = %f; qmu2_d[1] = %f.\n", qmu2_d[0], qmu2_d[1]);
            // Rprintf("mu_d[0] = %f; mu_d[1] = %f.\n", mu_d[0], mu_d[1]);
            // Rprintf("mu2_d[0] = %f; mu2_d[1] = %f.\n", mu2_d[0], mu2_d[1]);
            // Rprintf("sigma2_d[0] = %f; sigma2_d[1] = %f.\n", sigma2_d[0], sigma2_d[1]);
            // update variance contributions
            // var tau
            var_tau += pow(mu_d[1] - mu_d[0], 2) / n;
            // Rprintf("BEFORE: var_tau = %f, var_tau_treated = %f\n", var_tau, var_tau_treated);
            ...
            // Rprintf("AFTER: var_tau = %f, var_tau_treated = %f\n", var_tau, var_tau_treated);
            // var sigma pi
            ...
            // Rprintf("var_sigmapi = %f, var_sigmapi_treated = %f\n", var_sigmapi, var_sigmapi_treated);
    ...
    // variance components
    // Rprintf("*(thread_args->var_tau) before = %f\n", *(thread_args->var_tau));
    *(thread_args->var_tau) += var_tau;
    // Rprintf("*(thread_args->var_tau) after = %f\n", *(thread_args->var_tau));
    // Rprintf("*(thread_args->var_tau_treated) before = %f\n", *(thread_args->var_tau_treated));
...
int cm_variance_estimator(CMModelKnownPropscore *cm_model, CMResults *results){
    ...
    if (NUM_THREADS > 1){ // multithreading
        ...
        for (long t = 0; t < NUM_THREADS; t++){
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_variance_estimator`: creating thread %ld.\n", t);
            }
            thread_args[t].threadid = t;
            thread_args[t].cm_model = cm_model;
            thread_args[t].results = results;
            ...
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_variance_estimator`: thread %ld index_high is %zu.\n", t, thread_args[t].index_high);
            }
        ...
        for (long t = 0; t < NUM_THREADS; t++){
            ...
            assert(!result_code);
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_variance_estimator`: thread %ld has finished.\n", t);
            }
        }
    } else { // single thread
    ...
    if (CM_VERBOSE_MODE){
        Rprintf("var_tau_treated after = %f\n", var_tau_treated);
    }
    var_tau -= pow(results->ate_hat, 2);
    var_tau_treated -= pow(results->att_hat, 2) / p1_hat;
    if (CM_VERBOSE_MODE){
        Rprintf("var_tau_treated after after = %f\n", var_tau_treated);
        Rprintf("results->att_hat = %f\n", results->att_hat);
        Rprintf("pow(results->att_hat, 2) = %f\n", pow(results->att_hat, 2));
        Rprintf("p1_hat = %f\n", p1_hat);
        Rprintf("var_tau after after = %f\n", var_tau);
    }
...
double cm_maxmin_delta(vector_short *d, vector *propscore, size_t *pscore_sortindex){
    ...
    if (NUM_THREADS > 1){   // multi-threaded version
        ...
        for (long t = 0; t < NUM_THREADS; t++){
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_maxmin_delta`: creating thread %ld.\n", t);
            }
            thread_args[t].threadid = t;
            thread_args[t].d = d;
            ...
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_maxmin_delta`: thread %ld sortindex_high is %zu.\n", t, thread_args[t].sortindex_high);
            }
        ...
        for (long t = 0; t < NUM_THREADS; t++){
            ...
            assert(!result_code);
            if (CM_VERBOSE_MODE){
                Rprintf("`cm_maxmin_delta`: thread %ld has finished.\n", t);
            }
        }
...
void test_cm_maxmin_delta(void){
    ...
    // max| .........................       0.4
    maxmin = 0.4;
    // Rprintf("cm_maxmin_delta(d, propscore, pscore_sortindex) = %f\n", cm_maxmin_delta(d, propscore, pscore_sortindex));
    ...
    Rprintf("Tests for `cm_maxmin_delta` completed: no issues found.\n");
}
...
int cm_initialise_known_propscore(CMModelKnownPropscore *cm_model){
    ...
    if (cm_cmmodelknownpropscore_init_check_values(cm_model, 0)){
        // exit(EXIT_FAILURE);
        error("`cm_cmmodelknownpropscore_init_check_values` failed.\n");
    };
    // sample size
    size_t n = cm_model->d->size;
...
CMResults *cm_cm_known_propscore(CMModelKnownPropscore *cm_model){
    ...
    if (cm_cmmodelknownpropscore_check_values(cm_model, 0)){
        // exit(EXIT_FAILURE);
        error("%s: line %d: `cm_cmmodelknownpropscore_check_values` failed.\n", __FILE__, __LINE__);
    };
    // ensure model has been instantiated
    if (!cm_model->_isinstantiated){
        error("%s: line %d: CMModelKnownPropscore has not been instantiated prior to calling `cm_cm_known_propscore`. Do so with `cm_initialise_known_propscore`.\n", __FILE__, __LINE__);
        // exit(EXIT_FAILURE);
    }
    CMResults *results = malloc(sizeof(CMResults));
    ...
    cm_matches(cm_model, results);
    // some sanity checks
    // Rprintf("delta = %f", cm_model->delta);
    ...
    //             Rprintf("; p[%d] = %.3f, p[%d] = %.3f", i, vector_get(cm_model->propscore, i), j, vector_get(cm_model->propscore, j));
    //         }
    //     }
...
int cm_initialise(CMModel *cm_model){
    ...
    if (cm_cmmodel_init_check_values(cm_model, 0)){
        error("%s: line %d: `cm_cmmodel_init_check_values` failed.\n", __FILE__, __LINE__);
        // exit(EXIT_FAILURE);
    };
    // sample size
    size_t n = cm_model->d->size;
...
CMResults *cm_cm(CMModel *cm_model){
    // ensure model has been instantiated
    if (!cm_model->_isinstantiated){
        error("%s: line %d: CMModel has not been instantiated prior to calling `cm_cm`. Do so with `cm_initialise`.\n", __FILE__, __LINE__);
        // exit(EXIT_FAILURE);
    }
    // validate inputs
    if (cm_cmmodel_check_values(cm_model, 0)){
        error("%s: line %d: `cm_cmmodel_check_values` failed.\n", __FILE__, __LINE__);
        // exit(EXIT_FAILURE);
    };
...
void test_cm(void){
    ...
    Rprintf("=== Tests: all tests for `cm` completed: no issues found.\n");
}