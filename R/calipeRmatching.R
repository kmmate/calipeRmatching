# R interface to functions in src/calipeRmatching.cpp



#' Sets number of parallel threads for calipeRmatching library.
#'
#' Sets the number of threads for calipeRmatching library.
#' calipeRmatching processes observations in a parallel way. The default number of threads used
#' for parallel execution is the number of logical cores on the machine. That can be overwritten by
#' calling \code{cm_set_number_of_threads} passing as argument the desired number of threads.
#' Calling the function takes effect at any point of the program, not only upon loading the library.
#' 
#' @param nr_threads the desired number of threads. 
#' 
#' @return Zero if the number of threads is set successfully. Otherwise, the program stops.
#' 
#' @author Mate Kormos
...
#'
#' @export 
cm_set_number_of_threads <- function(nr_threads){
    return(cpp_cm_set_number_of_threads(nr_threads))
}
...
#'
#' Runs the test suite of the calipeRmatching library.
#' 
...
#' 
#' ###############################################################################
#' # Test if calipeRmatching library works as intended
#' ###############################################################################
#' test_cm()
#'
#' @export
...
#' Caliper matching estimator for known propensity score.
#'
#' 
#' 
#' @description 
...
#'
#'
#' @param y \eqn{n}-long \code{numeric} \code{vector} \eqn{ (Y_1,Y_2,\ldots,Y_n) } of observations of the outcome variable.
#' @param d \eqn{n}-long \code{numeric} \code{vector} \eqn{ (D_1,D_2,\ldots,D_n) } of observations of the treatment indicator
#'  variable. Each entry can be either 0 for control or 1 for treatment status.
#' @param propscore \eqn{n}-long \code{numeric} \code{vector} \eqn{ (\pi(X_1),\pi(X_2),\ldots,\pi(X_n)) } of observations of the propensity score.
...
#'
#' @seealso \code{\link{cm_cm_estimated_propscore}} 
#' @author Mate Kormos
#' @references [KPV23] Kormos, V. d. Pas, V. d. Vaart (2023): Asymptotics of Caliper Matching Estimators for Average Treatment Effects, https://arxiv.org/abs/2304.08373
#' @examples
#'
...
cm_cm_known_propscore <- function(y, d, propscore, delta, estimate_variance = TRUE, beta = 1/4.000000001, alpha = 1/4.000000002, kappa_a = 10^(-15), kappa_gamma = 0.2){
    ...
    if (!is.numeric(y)){
        stop("`y` must be of numberic type.")
    }
    if (!is.numeric(d)){
        stop("`d` must be of numberic type.")
    }
    if (!is.numeric(propscore)){
        stop("`propscore` must be of numberic type.")
    }
    if (!is.numeric(delta)){
        stop("`delta` must be of numberic type.")
    }
    if (!is.logical(estimate_variance)){
        stop("`estimate_variance` must be of logical type.")
    }
    if (!is.numeric(beta)){
        stop("`beta` must be of numberic type.")
    }
    if (!is.numeric(alpha)){
        stop("`alpha` must be of numberic type.")
    }
    if (!is.numeric(kappa_a)){
        stop("`kappa_a` must be of numberic type.")
    }
    if (!is.numeric(kappa_gamma)){
        stop("`kappa_gamma` must be of numberic type.")
    }
    # --- check if vector
    if (!is.vector(y)){
    ...
    if (!is.vector(d)){
        stop("`d` must be a vector.")
    }
    if (!is.vector(propscore)){
        stop("`propscore` must be a vector.")
    }
    # --- check if has right length
    n <- length(y)
    if (!((length(d) == n) && (length(propscore) == n))){
        stop("length of `y`, `d` and `propscore` must be equal.")
    }


    # call version from calipeRmatching.cpp
    return(cpp_cm_cm_known_propscore(y, d, propscore, delta, estimate_variance, beta, alpha, kappa_a, kappa_gamma))
}
...
#' Caliper matching estimator for estimated propensity score.
#'
#' @description 
...
#' @param d \eqn{n}-long \code{numeric} \code{vector} \eqn{ (D_1,D_2,\ldots,D_n) } of observations of the treatment indicator
#'  variable. Each entry can be either 0 for control or 1 for treatment status.
#' @param x \eqn{n}-by-\eqn{k} \code{numeric} \code{array} of \eqn{n} observations of the \eqn{k} covariates \eqn{ (X_i)_{i\in[n]} \in R^{n\times k} }, \emph{not including} an intercept.
#' @param modeltype type of the generalised linear propensity score model \eqn{g}. Currently only "logit" (\eqn{g} is the logistic distribution function) or "probit" (\eqn{g} is the normal distribution function) are supported.
#' @param theta_hat estimated \eqn{(k+1)}-long coefficient vector \eqn{\hat\theta} of the propensity score model. 
...
#'}
#' 
#' @seealso \code{\link{cm_cm_known_propscore}}
#' @author Mate Kormos
#' @references [KPV23] Kormos, V. d. Pas, V. d. Vaart (2023): Asymptotics of Caliper Matching Estimators for Average Treatment Effects, https://arxiv.org/abs/2304.08373
#' @examples
#'
...
cm_cm_estimated_propscore <- function(y, d, x, modeltype, theta_hat, delta, estimate_variance = TRUE, beta = 1/4.000000001, alpha = 1/4.000000002, kappa_a = 10^(-15), kappa_gamma = 0.2, kappa_gamma_derivative = 0.5){
    ...
    if (!is.numeric(x)){
        stop("`x` must be of numberic type.")
    }
    if (!is.character(modeltype)){
        stop("`modeltype` must be of character type.")
    }
    if (!is.numeric(theta_hat)){
        stop("`theta_hat` must be of numberic type.")
    }
    if (!is.numeric(delta)){
        stop("`delta` must be of numberic type.")
    }
    ...
    if (!is.numeric(kappa_gamma_derivative)){
        stop("`kappa_gamma_derivative` must be of numberic type.")
    }
    ...
    # --- check if dimensions/lengths are right
    n <- length(y)
    if (!((length(d) == n) && (nrow(x) == n))){
        stop("length of `y`, `d` must be equal to the number of rows in `x`.")
    }
    k <- ncol(x)
    if (!( length(theta_hat) == k + 1)){
        stop("length of `theta_hat` must be exactly one larger than the number of columns of `x`.")
    }



    # call version from calipeRmatching.cpp
    return(cpp_cm_cm(y, d, x, modeltype, theta_hat, delta, estimate_variance, beta, alpha, kappa_a, kappa_gamma, kappa_gamma_derivative))
}